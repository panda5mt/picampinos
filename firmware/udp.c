...
void __time_critical_func(udp_packet_gen)(uint32_t *buf, uint8_t *udp_payload) {
    ...
    dma_channel_wait_for_finish_blocking(DMA_UDP);  // 霆｢騾∝ｮ御ｺ・ｾ・ｩ・
    ...
    // DMA霆｢騾√↓繧医ｋCRC貍皮ｮ・
    channel_config_set_read_increment(&c0, true);
    channel_config_set_write_increment(&c0, false); // 霆｢騾∝・縺ｯNULL縺ｮ縺溘ａ繧､繝ｳ繧ｯ繝ｪ荳崎ｦ・
    dma_channel_configure (
        DMA_UDP,                // Channel to be configured
        &c0,                    // The configuration we just created
    ...
    //dma_sniffer_set_byte_swap_enabled(true);              // 1Byte蜊倅ｽ阪・霆｢騾√↑縺ｮ縺ｧSwap縺ｯ縺ｪ縺上※繧０K
    hw_set_bits(&dma_hw->sniff_ctrl,                        // 縺翫∪縺倥↑縺・
               (DMA_SNIFF_CTRL_OUT_INV_BITS | DMA_SNIFF_CTRL_OUT_REV_BITS));
    dma_hw->sniff_data = 0xffffffff;                        // CRC繧ｷ繝ｼ繝牙・譛溷喧
    dma_channel_set_read_addr(DMA_UDP, &data_8b[8], true);  // 霆｢騾・幕蟋・
    dma_channel_wait_for_finish_blocking(DMA_UDP);          // 霆｢騾∝ｮ御ｺ・ｾ・ｩ・
    uint32_t crc = dma_hw->sniff_data;                      // CRC貍皮ｮ礼ｵ先棡蜿門ｾ・
